    ...
    k.module("ngAria", ["ng"]).provider("$aria", function() {
        function a(a, e, b) {
            return function(f, d, h) {
                var l = h.$normalize(e);
                c[l] && !h[l] && f.$watch(h[a], function(a) {
                    b && (a = !a);
                    d.attr(e, a)
                })
            }
        ...
        return {
            ...
            link: function(e, b, f, d) {
                ...
                var n = g(f, b),
                    m = c("tabindex", "tabindex", b);
                switch (n) {
                    case "radio":
                    case "checkbox":
                        c("aria-checked", "ariaChecked", b) && e.$watch(h, "radio" === n ? l() : k);
                        break;
                    case "range":
                        a.config("ariaValue") && (f.min && !b.attr("aria-valuemin") && b.attr("aria-valuemin", f.min),
                            f.max && !b.attr("aria-valuemax") && b.attr("aria-valuemax", f.max), b.attr("aria-valuenow") || e.$watch(h, function(a) {
                ...
                m && b.attr("tabindex", 0);
                d.$validators.required && c("aria-required", "ariaRequired", b) && e.$watch(function() {
        ...
        return {
            restrict: "A",
            require: "?ngMessages",
            link: function(a, c, g, e) {
                c.attr("aria-live") || c.attr("aria-live", "assertive")
            }
        }
    }).directive("ngClick", ["$aria", function(a) {
        return {
            restrict: "A",
            link: function(c, g, e) {
                a.config("tabindex") && !g.attr("tabindex") && g.attr("tabindex", 0);
                if (a.config("bindKeypress") && !g.attr("ng-keypress")) g.on("keypress", function(a) {
                    32 !== a.keyCode &&
                        13 !== a.keyCode || c.$eval(e.ngClick)
                })
            }
        }
